...
<form class="flex flex-col">
    <div class="grid gap-6 mb-6 md:grid-cols-2">
        <div>
            ...
            <Input type="text" id="first_name" disabled={is_booked} placeholder="Joel" value={f_name} required />
        </div>

        <div>
            <Label for="last_name">Efternamn</Label>
            <Input type="text" id="last_name" disabled={is_booked} placeholder="Joelsson" value={l_name} required />
        </div>

        <div>
            <Label for="email">Emejl</Label>
            <Input type="email" id="email" disabled={is_booked} placeholder="joel@joel.se" value={email} required />
        </div>

        <div>
            <Label for="tel">Telefonnummer</Label>
            <Input type="tel" id="tel" disabled={is_booked} placeholder="0703180710" value={tel} required />
        </div>
    </div>
    {#if !is_booked}
        <Button type="submit" color="blue" on:click={handleSubmit}>Bokad</Button>
    {:else}
